...
public class UniMasterChef {
    ...
    // for addRating, maximum of entries is defined in constructor
    private int counter;
    ...
    public boolean addEntry(CompetitionEntry entry) {
        if (this.entries.length == counter) return false; // Maximum entries

        for (CompetitionEntry e : entries) {
            if (e != null && e.equals(entry)) return false;
        }
    ...
    public Dessert getBestDessert() {
        ...
        return entries[maxId].getDessert();
    }

    public static Person[] getInvolvedPeople(CompetitionEntry entry) {
        Person[] returnVal = new Person[4];

        int counter = 0;

        returnVal[counter++] = entry.getTeacher();
